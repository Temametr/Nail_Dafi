...
export function renderTimeSlots(occupiedSlots) {
    ...
    container.innerHTML = slots.map((time, i) => {
        ...
        // Перевірка 1: Чи не вилазить послуга за межі 20:00
        if (i + reqBlocks > slots.length) {
            isAvail = false; 
        } else {
            // Перевірка 2: Чи не зайнятий час
        ...
        // Перевірка 3: Чи не минув час (якщо запис на сьогодні)
    ...
    if (availableCount === 0) {
        container.innerHTML = '<div class="col-span-4 text-center text-slate-500 py-6 font-medium bg-white rounded-2xl border border-slate-100 shadow-convex-sm">На жаль, вільного часу немає 😔</div>';
    }
}
...
export function renderUserProfile() {
    ...
    // Захист від помилок, якщо Telegram API ще не підтягнув дані
