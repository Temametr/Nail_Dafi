...
import { fetchOccupiedSlotsAPI } from './api.js';
...
export function renderHomeMasters() {
    ...
    list.innerHTML = reversedMasters.map((m, i) => {
        ...
        return `
        <div class="relative w-full aspect-[3/4] rounded-3xl overflow-hidden shadow-convex animate-pop-in border-2 border-white/60" style="animation-delay: ${i*50}ms">
            ...
            <div class="absolute bottom-2.5 left-2.5 right-2.5 bg-white/85 backdrop-blur-md rounded-2xl p-2.5 shadow-lg border border-white/60 text-center flex flex-col justify-center items-center">
                ...
                <p class="text-[8px] font-bold text-rose-500 uppercase tracking-widest">Топ-мастер</p>
            </div>
        </div>
        `;
    }).join('');
        ...
        <div onclick="window.appAPI.selectService('${s.id}')" class="card-convex p-5 mb-4 flex justify-between items-center active:scale-95 transition-all duration-300 cursor-pointer shadow-convex animate-pop-in border border-white" style="animation-delay: ${i*40}ms">
            <div class="flex items-center gap-4 flex-1 min-w-0 pr-2">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs font-semibold text-slate-500 mt-1 flex items-center gap-1.5"><span class="w-4 h-4 bg-slate-100 rounded-full flex items-center justify-center text-slate-400">🕒</span> ${s.duration} мин</div>
                </div>
            </div>
        ...
        <div onclick="window.appAPI.selectMaster('${m.id}')" class="card-convex p-5 mb-4 flex justify-between items-center active:scale-95 transition-all duration-300 cursor-pointer shadow-convex animate-pop-in border border-white" style="animation-delay: ${i*40}ms">
            <div class="flex items-center gap-4 flex-1 min-w-0">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="text-[10px] font-bold text-slate-500 mt-1 uppercase tracking-wider">График: ${m.workHours}</div>
                </div>
            </div>
...
export function renderClientBookings() {
    ...
    if (filtered.length === 0) { container.innerHTML = "<div class='text-center py-12 text-slate-400 font-medium'>У вас пока нет записей.</div>"; return; }
    ...
    container.innerHTML = filtered.map((b, i) => {
        ...
        let masterName = masterObj ? masterObj.name : 'Мастер';
                ...
                <div class="flex justify-between items-start mb-4">
                    <div class="w-full pr-3">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="text-sm font-semibold text-slate-600 flex items-center gap-2.5"><span class="w-7 h-7 rounded-full bg-rose-50 flex justify-center items-center text-rose-500 text-[10px]">💅</span> Мастер: ${masterName}</div>
                        </div>
                    </div>
                    <span class="text-[10px] font-bold px-3 py-1.5 rounded-full border shrink-0 ${statusData.color}">${statusData.text}</span>
                </div>
                ${b.cancelReason ? `<div class="text-xs text-red-700 mt-4 bg-red-50 p-4 rounded-2xl border border-red-100 font-medium leading-relaxed">Комментарий: ${b.cancelReason}</div>` : ''}
                ${(isPending || isConfirmed) ? `<div class="mt-4 pt-4 border-t border-slate-100 space-y-3.5">
                    ${isConfirmed ? `
                        <button onclick="tg.showAlert('Функция чата скоро появится! Пишите в личные сообщения.')" class="card-convex-sm flex items-center justify-center w-full py-3.5 bg-slate-950 text-white rounded-xl text-sm font-bold shadow-lg active:scale-95 transition-all">Написать мастеру 💬</button>
                        <div class="flex gap-3">
                            ...
                            <button onclick="window.appAPI.openCancelModal('${b.id}', 'client')" class="card-convex-sm flex-1 py-3 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl text-sm font-bold active:scale-95 transition-all border border-red-100">Отменить</button>
                        </div>` : `
                        <button onclick="window.appAPI.openCancelModal('${b.id}', 'client')" class="card-convex-sm w-full py-3.5 bg-white text-slate-600 hover:bg-slate-100 rounded-xl text-sm font-bold active:scale-95 transition-all border border-slate-200">Отменить визит</button>
                    `}</div>` : ''}
            </div>
        `;
    }).join('');
